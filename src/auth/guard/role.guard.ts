...
export class RolesGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        // Lấy thông tin từ request (gồm token, role_name)
        const req = context.switchToHttp().getRequest();
        const tokenReq = req.headers.authorization.slice(7);
        const userRoleName = req.user.data.role.role_name;
        ...
        if (tokenReq === tokenFromDB) {
            // Nếu từ token user trả về thuộc 1 trong các quyền từ @RoleDecorator(Role.ADMIN, Role.USER)
            //  return => true/false
            return requiredRoles.some((role) => role === userRoleName); // true
        } else {
            return false;
        }
    }
